...
----------------------------------------------------------------------"""
def makeNewOutputs(correlation, # corr dataframe  
                   clusters, # cluster 1
                   clusters2): # cluster 2
    # merge two clusters
    clustersNew={}
    ...
    distance = ((1 - correlation.fillna(0))/2.)**.5 # distance dataframe
...
methodology: Snipet 4.2, Page 58
----------------------------------------------------------------------"""
def clusterKMeansTop(correlation, # corr dataframe  
                     numberClusters = None, # number of clusters
                     iterations = 10): # number of iterations
    if numberClusters == None:numberClusters = correlation.shape[1] - 1 # set number of cluster
    # clustering
    correlationSorted, clusters, silh = clusterKMeansBase(correlation, numberClusters = min(numberClusters, correlation.shape[1] - 1), iterations = iterations)
    clusterTstats = {i:np.mean(silh[clusters[i]])/np.std(silh[clusters[i]]) for i in clusters.keys()} # calculate t-stat of each cluster
    tStatMean = sum(clusterTstats.values())/len(clusterTstats) # calculate mean of t-stats
    redoClusters = [i for i in clusterTstats.keys() if clusterTstats[i] < tStatMean] # select clusters which have t-stat lower than mean
    if len(redoClusters) <= 1:
        ...
        correlationSorted2, clusters2, silh2 = clusterKMeansTop(correlationTemp, numberClusters = min(numberClusters, correlationTemp.shape[1] - 1), iterations = iterations)
        # Make new outputs, if necessary
        correlationNew, clustersNew, silhNew = makeNewOutputs(correlation, {i:clusters[i] for i in clusters.keys() if i not in redoClusters}, clusters2)
        # mean of t-stats new output
        newTstatMean = np.mean([np.mean(silhNew[clustersNew[i]])/np.std(silhNew[clustersNew[i]]) for i in clustersNew.keys()])
...
----------------------------------------------------------------------"""
def randomCovarianceSub(numberObservations, # number of observations
    ...
    if numberColumns == 1:return np.ones((1, 1))
...
methodology: Snipet 4.3, Page 61
----------------------------------------------------------------------"""
def randomBlockCovariance(numberColumns, # number of cols
    ...
    cov = None
    for column in parts:
        thisCovariance = randomCovarianceSub(int(max(column*(column + 1)/2., 100)), column, sigma, randomState = domain) # sub covariance
        if cov is None:cov = thisCovariance.copy() # copy covariance matrix
        else:cov = block_diag(cov, thisCovariance) # block diagram covariance matrix
    return cov
...
methodology: Snipet 4.3, Page 61
----------------------------------------------------------------------"""
def randomBlockCorrelation(numberColumns,  # number of cols
                           numberBlocks, # number of blocks
                           randomState = None,  # for rand data
                           blockSizeMin = 1): #  minimum size of block
    # Form block corr
    domain = check_random_state(randomState)
    # generate 2 random block diagram cov matrix
    covariance1 = randomBlockCovariance(numberColumns, numberBlocks, blockSizeMin = blockSizeMin, sigma = .5, randomState = domain)
    covariance2 = randomBlockCovariance(numberColumns, 1, blockSizeMin = blockSizeMin, sigma = 1., randomState = domain) # add noise
