...
class AdaptiveCombinatorialPurged(CombinatorialPurged):
    ...
    def __init__(
        ...
        if not self.is_multiple_datasets and not isinstance(
    ...
    def _validate_input(
        ...
        if not single_data.index.equals(single_external_feature.index):
            raise ValueError("Data and external_feature must have the same index")
    ...
    def _single_adaptive_split_segments(
        ...
        subsplit_starts_loc = np.array(
            [split[0] for split in subsplits if len(split) > 0]
        )
    ...
    def _single_split(
        ...
        split_segments = self._get_split_segments(single_data, single_external_feature)
    ...
    def _single_backtest_predictions(
        ...
        combinatorial_trained_estimators = Parallel(n_jobs=n_jobs)(
            delayed(train_single_estimator)(clone(single_estimator), test_indices)
    ...
    def backtest_predictions(
        ...
        if not (isinstance(data, pd.DataFrame) and isinstance(labels, pd.Series)):
            raise ValueError(
