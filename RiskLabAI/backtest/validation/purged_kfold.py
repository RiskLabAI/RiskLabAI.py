...
class PurgedKFold(CrossValidator):
    ...
    def filtered_training_indices_with_embargo(
        ...
        if embargo_length == 0:
            ...
            test_data_info_range = data_info_range[effective_test_time_range.index.min():]
            embargoed_data_info_range = pd.Series(
                test_data_info_range.iloc[embargo_length:],
                index=test_data_info_range.iloc[:-embargo_length]
            )
            embargoed_data_info_range = pd.concat([
                embargoed_data_info_range,
                pd.Series(test_data_info_range[-1], index=test_data_info_range[-embargo_length:])
            ])
    ...
    def backtest_paths(
        ...
        :param data: Dataset or dictionary of datasets.
        :type data: Union[pd.DataFrame, Dict[str, pd.DataFrame]]
        :return: Backtest paths structured in a dictionary. For multiple datasets, an outer dictionary is used with 
                dataset names as keys and individual backtest path dictionaries as values.
        :rtype: Union[
            Dict[str, List[Dict[str, np.ndarray]]],
            Dict[str, Dict[str, List[Dict[str, List[np.ndarray]]]]]
        ]
