...
class PurgedKFold(CrossValidator):
        ...
        if embargo_length == 0:
            ...
            for end_val in effective_test_time_range.values:
                ...
                if end_iloc >= len(data_info_range):
                    ...
                    embargoed_iloc = min(
                        end_iloc + embargo_length, len(data_info_range) - 1
                    )
                    embargoed_values.append(data_info_range.index[embargoed_iloc])

            embargoed_ranges = pd.Series(
                embargoed_values, index=effective_test_time_range.index
            )
            # === END OF FIX ===
    ...
    def _validate_input(
        self, single_times: pd.Series, single_data: pd.DataFrame
    ) -> None:
        """Validate that data and times indices match."""
        if not single_data.index.equals(single_times.index):
            raise ValueError("Data and 'times' (info range) must have the same index")
    ...
    def split(
        ...
        if self.is_multiple_datasets:
            ...
            for train_indices, test_indices in self._single_split(self.times, data):
                yield train_indices, test_indices
