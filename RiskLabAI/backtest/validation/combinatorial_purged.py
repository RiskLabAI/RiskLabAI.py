...
class CombinatorialPurged(PurgedKFold):
    ...
    def backtest_paths(
        ...
        """
        Generate backtest paths for the given data and labels.

        This function can handle both single and multiple datasets and creates multiple 
        backtest paths based on combinatorial splits, where each path represents a sequence
        ...
        .. note:: This function relies on internal methods (e.g., `_single_backtest_paths`) 
                to generate the backtest paths for each dataset.
        """
    ...
    def backtest_predictions(
        ...
        """
        Conduct backtest predictions on given data using specified estimator(s).

        This function uses either a single estimator or multiple estimators (provided as a dictionary) 
        to generate predictions on the input data. It supports both single and multiple datasets 
        scenarios based on the `is_multiple_datasets` attribute.

        :param estimator: Machine learning model or models (as dictionary) to be used for predictions.
        :param data: Input data as DataFrame or a dictionary of DataFrames for multiple datasets.
        :param labels: Target labels as Series or a dictionary of Series for multiple datasets.
        :param sample_weights: Optional sample weights or a dictionary of sample weights for multiple datasets.
        :param predict_probability: If True, predict the probability of forecasts. 
        :param n_jobs: Number of CPU cores to use for parallel processing. Default is 1.

        :return: A dictionary of predictions. If multiple datasets, a nested dictionary where the first key
                is the dataset name, and the inner dictionary contains predictions.

        .. note:: This function internally calls `_single_backtest_predictions` which handles the prediction
                logic for each dataset separately.
        """
        if self.is_multiple_datasets:
