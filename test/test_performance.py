...
def test_mp_avg_active_signals_matches_reference():
    ...
    time_points = sorted(
        set(signals["t1"].dropna().values).union(signals.index.values)
    )
...
def _triple_barrier_reference(close, events, ptsl, molecule):
    ...
    pt = ptsl[0] * ef["Base Width"] if ptsl[0] > 0 else pd.Series(np.inf, index=ef.index)
    sl = -ptsl[1] * ef["Base Width"] if ptsl[1] > 0 else pd.Series(-np.inf, index=ef.index)
...
def test_triple_barrier_matches_reference_randomized():
    ...
    for _ in range(25):
        ...
        close = pd.Series(np.cumprod(1 + rng.standard_normal(n) * 0.01) * 100, index=idx)
