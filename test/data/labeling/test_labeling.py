...
def price_series():
    ...
    dates = pd.to_datetime(pd.date_range("2020-01-01", periods=20, freq="D"))
    prices = [
        10,
        11,
        12,
...
def test_cusum_filter_dynamic_threshold(price_series):
    ...
    expected_dates = pd.to_datetime(["2020-01-04", "2020-01-09", "2020-01-18"])
    pd.testing.assert_index_equal(events, expected_dates)
...
def test_meta_events_and_labeling(price_series):
    ...
    events = meta_events(
        price_series, time_events, ptsl, volatility, return_min, num_threads
    )
