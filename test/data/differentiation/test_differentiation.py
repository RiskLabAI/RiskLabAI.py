...
def test_fractional_difference_std(sample_series):
    ...
    assert np.allclose(
        diff_std.loc[common_idx, "close"], diff_pd.loc[common_idx, "close"]
    )
...
def test_fractional_difference_fixed(sample_series):
    ...
    assert np.allclose(
        diff_ffd.loc[common_idx, "close"], diff_pd.loc[common_idx, "close"]
    )
...
def test_fractional_difference_fixed_single(sample_series):
    ...
    common_idx = diff_ffd.index.intersection(diff_pd.index)
    assert np.allclose(diff_ffd.loc[common_idx], diff_pd.loc[common_idx])
