...
def test_average_bet_sizes_numba():
    ...
    avg_sizes = average_bet_sizes(price_dates, start_dates, end_dates, bet_sizes)
    ...
    # Date 8: [8, 9] -> -1.0
    # Date 9: [8, 9] -> -1.0
    expected = np.array([1.0, 1.0, 0.75, 0.75, 0.75, 0.75, 0.5, 0.5, -1.0, -1.0])
...
def test_strategy_bet_sizing():
    ...
    bet_idx = pd.to_datetime(["2020-01-01", "2020-01-03", "2020-01-09"])

    times = pd.Series(
        pd.to_datetime(["2020-01-06", "2020-01-08", "2020-01-10"]), index=bet_idx
    )
