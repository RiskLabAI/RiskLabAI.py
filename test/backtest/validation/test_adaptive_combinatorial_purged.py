...
from RiskLabAI.backtest.validation.adaptive_combinatorial_purged import (
...
def sample_data_with_times_and_feature():
    ...
    X = pd.DataFrame({"feature1": np.arange(n_samples)}, index=idx)
    y = pd.Series(np.random.randint(0, 2, n_samples), index=idx)
    times = pd.Series(idx + pd.DateOffset(days=7), index=idx)
    # External feature: a sine wave to create predictable quantiles
    external_feature = pd.Series(np.sin(np.linspace(0, 10, n_samples)), index=idx)
...
def test_adaptive_predictions(sample_data_with_times_and_feature):
    ...
    cv = AdaptiveCombinatorialPurged(
        n_splits=6, n_test_groups=2, times=times, external_feature=feature, embargo=0.01
    )
