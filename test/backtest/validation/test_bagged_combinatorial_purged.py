...
def sample_data_with_times():
    ...
    y_class = pd.Series(np.random.randint(0, 2, n_samples), index=idx)
    y_reg = pd.Series(np.random.randn(n_samples), index=idx)
...
def test_bagged_init(sample_data_with_times):
    ...
    cv = BaggedCombinatorialPurged(
        n_splits=6, n_test_groups=2, times=times, classifier=True, n_estimators=5
    )
...
def test_bagged_predictions_regressor(sample_data_with_times):
    ...
    with pytest.raises(ValueError):
        cv.backtest_predictions(model, X, y_reg, predict_probability=True, n_jobs=1)
