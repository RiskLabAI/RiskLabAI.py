...
def sample_data_with_times():
    ...
    times = pd.Series(X.index + pd.DateOffset(days=7), index=X.index)
    return X, y, times
...
def test_combinatorial_purged_init(sample_data_with_times):
    ...
    cv = CombinatorialPurged(
        n_splits=n_splits, n_test_groups=n_test_groups, times=times
    )
...
def test_combinatorial_split(sample_data_with_times):
    ...
    n_test_groups = 2
    cv = CombinatorialPurged(
        n_splits=n_splits, n_test_groups=n_test_groups, times=times, embargo=0.01
    )
